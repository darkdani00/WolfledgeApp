...
export class RestService {
  apiUrl = "http://localhost/FindingClassesApi/index.php/";
  ...
  async login(_data : any){
    ...
    return this.http.post<any>(this.apiUrl+"usuario/api/login",_data).subscribe(result =>{
      ...
      console.info(err);
      this.display_toast('Error',"danger","Error de comunicaciónes, intente más tarde",'top',4000);
      loading.dismiss();
    });
  }
  ...
  async cita(_data : any){
    ...
    await loading.present();
    console.info(this.apiUrl);
    return this.http.post<any>(this.apiUrl+"users/api/citas",_data).subscribe(result =>{
      loading.dismiss();
  ...
  isLoggedIn(){
    this.storage.get('sw14_sess').then((response) => {
      if(response){
        this.authState.next(true);
      }
  ...
  authUserData(){
    return this.storage.get('sw14_sess');
  }
